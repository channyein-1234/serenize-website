...
export default async function handler(req, res) {
  ...
  try {
    console.log('Test handler called, user_id:', user_id);

    // Return a fixed test message instead of fetching data or calling OpenAI
    const testMessage = "This is a test suggestion. Your AI connection is working!";

    return res.status(200).json({ suggestions: testMessage });
  } catch (err) {
